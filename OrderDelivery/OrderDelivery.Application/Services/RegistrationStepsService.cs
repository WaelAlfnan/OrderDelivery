...
public class RegistrationStepsService : IRegistrationStepsService
{
    ...
    public async Task<PendingRegistration?> GetPendingRegistrationByPhoneAsync(string phoneNumber)
    {
        var repo = _unitOfWork.Repository<PendingRegistration>();
        return await repo.FirstOrDefaultAsync(x => x.PhoneNumber == phoneNumber);
    }
