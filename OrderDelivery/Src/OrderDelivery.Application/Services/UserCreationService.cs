...
public class UserCreationService : IUserCreationService
{
        ...
        string nationalIdNumber)
    {
        try
        {
            // Check if user with this username (phone number) already exists
            var existingUser = await _userManager.FindByNameAsync(phoneNumber);
            if (existingUser != null)
            {
                return (false, "User with this phone number already exists", null);
            }
            ...
            var user = new ApplicationUser
            {
                ...
                Email = string.Empty, // Set empty string instead of null to avoid validation issues
                FirstName = firstName,
                LastName = lastName,
                ...
                // Note: Email is set to empty string to avoid null reference issues
            };
